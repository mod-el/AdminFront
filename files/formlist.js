function getSublistArray(name) {
	...
	cont.querySelectorAll('input[type="hidden"][name^="ch-' + name + '-"]').forEach(mainInput => {
		...
		row.querySelectorAll('input[name^="ch-"], select[name^="ch-"], textarea[name^="ch-"]').forEach(input => {
			...
			rowArr[fieldName] = input.getValue(true);
		});
...
function saveFormList() {
	...
	let data = getSublistArray('list');
	let deleted = getDeletedFromSublist('list');
	return ajax(adminPrefix + currentAdminPage.split('/')[0] + '/saveFormList', 'ajax', 'data=' + encodeURIComponent(JSON.stringify(data)) + '&deleted=' + encodeURIComponent(JSON.stringify(deleted)) + '&c_id=' + c_id).then(r => {
		saving = false;
		restoreSaveButton();
